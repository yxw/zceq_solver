...
import platform
from . import DEFAULT_LIB_FILENAME 
...
def load_custom_library(library_filename):
    global library, ffi
    ...
    try:
        library_pathname = pkg_resources.resource_filename(__name__, library_filename)
...
def load_library():
    load_custom_library(DEFAULT_LIB_FILENAME)

class Solver:
    def __init__(self, library_filename=DEFAULT_LIB_FILENAME):
        self.solver_ = self.header_ = self.solutions_ = self.solution_to_check_ = None
        self.libfilename_ = library_filename
    ...
    def _ensure_library(self):
        ...
        if (library is None):
            load_custom_library(self.libfilename_)
